...
while true; do
	read -p "Give me a number from 1 to 10: " guess 

	[ -v guess ] && [ -z "$guess" ] && continue
	if [ $guess -ge 1 ] && [ $guess -le 10 ] ;then
		 [ $guess == $secretnumber ] &&  echo "You got it! Have a milkdud." && break 
		 [ $guess -le $secretnumber ] &&  echo "Too low!" || echo "Too high!"
	fi
done
